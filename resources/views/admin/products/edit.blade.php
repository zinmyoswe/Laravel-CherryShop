    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <input type="text" name="productid" id="productid" class="form-control" value="{{ $product->productid }}" required>
            </div>
        ...
        <div class="mb-3">
            ...
            <select name="sizes[]" id="sizes" class="form-select" multiple required>
                @foreach($sizes as $size)
                    <option value="{{ $size->id }}" {{ in_array($size->id, $product->sizes->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $size->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            @foreach ($products as $relatedProduct)
                <div class="form-check">
                    ...
                    @if(in_array($relatedProduct->id, $product->relatedProducts->pluck('id')->toArray())) checked @endif>
